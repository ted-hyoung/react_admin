...
// components
import { ProductDetail, CelebReviewDetail } from 'components';
...
function EventTab() {
  ...
  const getEvent = useCallback(() => {
    dispatch(getEventAsync.request(eventId));
  }, [dispatch]);
  ...
  return (
    <Tabs defaultActiveKey="product">
      ...
      <TabPane tab="제품 정보" key="product">
        <ProductDetail responseProducts={products} responseShippingFeeInfo={shippingFeeInfo} />
      </TabPane>
      <TabPane tab="셀럽 리뷰" key="celeb">
        <CelebReviewDetail id={1} />
      </TabPane>
