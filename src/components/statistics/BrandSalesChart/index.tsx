...
// types
import { ChartData, DataSets, ResponseManagementBrandStatistics } from 'models';

// less
import './index.less';
import { clearBrandSalesStatus } from 'store/reducer/brand';
...
const BrandSalesChart = (props: Props) => {
  ...
  return (
    <div
      ...
      // style={{width : '100%', height : '500px' ,overflowX : 'auto', overflowY : 'hidden' }}
    >
      <Bar
            ...
            callbacks: {
              title: (tooltipItem: any) => {
                ...
                if(tooltipItem[0].datasetIndex === 0){
                  return (
                    statisticsBrandSalesRow.brandName+' 순매출액 : ' +
                    (
                      statisticsBrandSalesRow.totalSalesAmount
                    ).toLocaleString() +
                    '원'
                  );
                }
                if(tooltipItem[0].datasetIndex === 1){
                  return (
                    statisticsBrandSalesRow.brandName+' 평균매출액 : ' +
                    (
                      statisticsBrandSalesRow.totalSalesAmountAvg
                    ).toLocaleString() +
                    '원'
                  );
                }
